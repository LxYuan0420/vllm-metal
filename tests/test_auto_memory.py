...
class TestAutoMemoryGuardrails:
    ...
    def test_legacy_worker_raises_when_minimal_needed_exceeds_total(self, monkeypatch) -> None:
        worker = LegacyMetalWorker.__new__(LegacyMetalWorker)
        worker.config = MetalConfig(
            memory_fraction=AUTO_MEMORY_FRACTION,
            use_mlx=False,
    ...
    def test_v1_worker_auto_memory_sets_limit_when_feasible(self, monkeypatch) -> None:
        ...
        assert 0 < captured.limit < 8_000_000_000

    def test_v1_worker_raises_when_minimal_needed_exceeds_total(self, monkeypatch) -> None:
        worker = V1MetalWorker.__new__(V1MetalWorker)
        worker.metal_config = MetalConfig(
            memory_fraction=AUTO_MEMORY_FRACTION,
            use_mlx=False,
